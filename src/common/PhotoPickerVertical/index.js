...
export default class PhotoPickerVertical extends PureComponent {
  ...
  renderResult(images: Array) {
    return (
      ...
      <View style={{flex:1}}>
      {
        images.map(uri => <View key={uri}>
            <Image resizeMode={Image.resizeMode.stretch} key={uri} source={{ uri, width: windowWidth - 20, height: 200 }} style={{marginTop:10}}/>
        <TouchableOpacity style={{position:'absolute', right:10,bottom:10,width:40,height:40,borderRadius:20,backgroundColor:'white',alignItems:'center',justifyContent:'center'}} key={uri+1} activeOpacity={0.6} underlayColor={'rgba(255,255,255,0.1)'} onPress={()=>{this.removePicture(uri)}}><View ><Image key={uri + 1} resizeMode='stretch' source={require('../Images/trash@3x.png')} style={{width:20,height:25}} ></Image></View></TouchableOpacity></View>)
      }
      </View>
  ...
  removePicture(uri)
  {
    const images = [...this.state.realSelected];
    images.splice(uri,1);
    console.log(images);
    this.setState(
      {
    ...
    this.removeDatasource(this.state.realDataSource,uri);
    this.onInitStates(this.state.dataSource,this.state.realDataSource);
    this.setState({
      dataSource:this.state.dataSource,
      realDataSource:this.state.realDataSource
    });
  ...
  static async requestAndroidPermission() {
    try {
      ...
      if (!granted || result.length > 0) {
        PhotoPickerVertical.alertErrorMsg('You cannot upload image');
      }
    } catch (err) {
      PhotoPickerVertical.alertErrorMsg(err);
    }
  }
  ...
  async componentDidMount() {
    ...
    if (Platform.OS === 'ios') {
      ...
      PhotoPickerVertical.requestAndroidPermission();
    }
  }
  ...
  removeDatasource(dataSource,uri)
  {
    const result = [];
    let temp = [];
    for (let k = 0,sz = dataSource.size;k < sz;++k)
  ...
  appendImages(data) {
    ...
    if (assets.length > 0) {
      ...
      const dataSource = this.state.dataSource.concat(
        Immutable.List(PhotoPickerVertical.numEveryRow(purgedData, 3))
      );
      const realDataSource = this.state.realDataSource.concat(
        Immutable.List(PhotoPickerVertical.numEveryRow(purgedData1, 3))
      );
  ...
  render() {
    ...
    return (
      <View style={Styles.wrapper}>
        ...
        <TouchableOpacity activeOpacity={0.6} underlayColor={'rgba(255,255,255,0.1)'} onPress={this.pickImage}>
            <Image resizeMode={Image.resizeMode.stretch} source={require('../Images/rectangle11Copy@3x.png')} style={{width:windowWidth - 20,height:200,marginTop:10}}/>
        </TouchableOpacity>
